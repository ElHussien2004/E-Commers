using DomainLayer.Contracts;
using DomainLayer.Models.BasketModuls;
using StackExchange.Redis;
using System;
using System.Collections.Generic;
using System.Data;
using System.Linq;
using System.Text;
using System.Text.Json;
using System.Threading.Tasks;

namespace Persistence.Repositories
{
    public class BasketRepository(IConnectionMultiplexer connection ) : IBasketRepository
    {
        private readonly IDatabase _database=connection.GetDatabase();
        public async Task<CustomerBasket?> CreateOrUpdateBasketAsync(CustomerBasket customerBasket, TimeSpan? TimeToLive = null)
        {
            var JsonBasket=JsonSerializer.Serialize(customerBasket);
          var IsCreatedOrDeleted= await  _database.StringSetAsync(customerBasket.Id ,JsonBasket ,TimeToLive ?? TimeSpan.FromDays(30));
            if (IsCreatedOrDeleted)
                return await GetBasketAsync(customerBasket.Id);
            else
                return null;
        }

        public async Task<bool> DelateBasketAsync(string id)
        {
             return await _database.KeyDeleteAsync(id);
        }

        public async Task<CustomerBasket?> GetBasketAsync(string Key)
        {
           var Basket =await _database.StringGetAsync(Key);
            if(Basket.IsNullOrEmpty)
            {
                return null;
            }
            else
            {
                return JsonSerializer.Deserialize<CustomerBasket>(Basket!);
            }
        }
    }
}
